...
- `NEXT-1`: Execute `M8-003` (outbox relay worker with bounded retry/backoff and dead-letter semantics).
- `NEXT-2`: Execute `M8-004` (async eval orchestration via worker-driven lifecycle transitions).
